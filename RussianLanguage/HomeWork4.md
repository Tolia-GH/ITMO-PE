...
- [HomeWork Main](#homework-main)
  ...
  - [](#)
  - [Актуальность научного исследования](#актуальность-научного-исследования)
    - [Актуальность](#актуальность)
    - [Цель работы:](#цель-работы)
    - [Задачи:](#задачи)
    - [Выводы и рекомендации:](#выводы-и-рекомендации)
  - [Аннотирование и Библиография\_ Продолжение](#аннотирование-и-библиография_-продолжение-1)
    - [Введение](#введение)
    - [Основные виды интернет-пиратства](#основные-виды-интернет-пиратства)
    - [Разновидности компьютерного пиратства](#разновидности-компьютерного-пиратства)
    - [Причины распространения пиратства](#причины-распространения-пиратства)
    - [Влияние интернет-пиратства на издательство](#влияние-интернет-пиратства-на-издательство)
    - [Необходимость реформ авторского права](#необходимость-реформ-авторского-права)
    - [Заключение](#заключение)
  - [Подготовка к КР по теме "Автобиография и резюме".](#подготовка-к-кр-по-теме-автобиография-и-резюме)
...
Задание 2. Соотнесите слова в левой и правой колонках. При необходимости измените форму существительного. Дайте все возможные варианты. (см.уч.РЯ в проф.д., с 100)

Актуальность исследования

Цель исследования заключается в (том, что/чтобы)
Цель исследования выражается в в (том, что/чтобы) 
Цель исследования является чем
Цель исследования состоит в том, что

Задачи исследования заключается в (том, что/чтобы)
Задачи исследования выражается в (том, что/чтобы) 
Задачи исследования является
Задачи исследования состоят в том, что

Объект исследования является
Объект исследования определяется чем?
Объект исследования служит чем? 

Предмет исследования является 
Предмет исследования определяется чем?
Предмет исследования служит чем?

Метод исследования заключается в том, чтобы
Метод исследования выражается в том, чтобы
Метод исследования состоит в том, что
Метод исследования достигается чем?
Метод исследования определяется чем?

Теоретическая значимость заключается в том, что
Теоретическая значимость выражается в том, что
Теоретическая значимость состоит в том, что
Теоретическая значимость определяется чем?

Практическая значимость заключается в том, что
Практическая значимость выражается в том, что
Практическая значимость состоит в том, что
Практическая значимость определяется чем?
Практическая значимость обусловлена тем, что

## Актуальность научного исследования

уч.РЯ в проф.д., с.91_Задание 1.

Задание 1. Прочитайте введение к курсовой работе. Найдите в нем основные содержательные части, а также информацию, которая должна быть отражена в основной части курсовой работы.  
任务 1. 阅读课程作业的简介。在其中找到主要内容部分，以及课程作业主要部分应体现的信息。

Введение 介绍

В данной курсовой будут рассматриваться вопросы программирования в системах компьютерной алгебры Maxima и Scilab.  
本课程将涵盖 Maxima 和 Scilab 计算机代数系统中的编程问题。
...
该主题的相关性在于，学校课程中实现编程算法化的路线数量不足。高级算法语言通常在学校教授，强大的 Maxima 和 Scilab 系统说明了直接面向用户编程的重要性，因为这些系统的开发人员认为有必要将编程工具留给用户来实现所有基本算法。这些系统可在教育领域免费使用，这使得它们对教育机构的使用具有吸引力。
...
Scilab 是一个跨平台的计算机代数系统（CAS），具有与 Matlab 类似的内置语言语法。 Scilab 系统的开发自上世纪80年代以来一直由法国国家信息学与自动化研究所（INRIA - Institut National de Recherche en Informatique et Automatique）的员工进行。
...
最初它是一个商业项目，名为 Blaise，后来又名为 Basile。自 2003 年起，该产品获得了新名称 Scilab 并免费。目前它是在免费的 CeCILL 许可证下分发的。
...
Scilab 系统本身与 Matlab 一样，主要用于数值计算和矩阵处理。此外，它还具有先进的编程工具（包括脚本调试器），因此在某种程度上它可以被视为开发高科技应用程序的系统。
...
系统有相当多的扩展包，可以在官网工具箱中心部分找到。然而，好的软件包越多，系统就会变得越有用。

Maxima – свободная система компьютерной алгебры, написанная на языке Common Lisp.  
Maxima 是一个用 Common Lisp 编写的免费计算机代数系统。
...
Maxima 源自 Macsyma 系统，该系统于 1968 年至 1982 年间在麻省理工学院开发，作为 MAC 项目的一部分，由美国能源部 (DOE) 和其他政府机构资助。德克萨斯大学奥斯汀分校的 William F. Schelter 教授从 1982 年一直支持该系统的一个版本，即 DOE Macsyma，直到他于 2001 年去世。
...
1998 年，Shelter 获得了能源部的许可，可以在 GPL 许可下发布 DOE Macsyma 源代码，并于 2000 年在 SourceForge.net 上创建了一个项目，以 Maxima 的名义维护和进一步开发 DOE Macsyma。

Maxima имеет широчайший набор средств для проведения аналитических вычислений, численных вычислений и построения графиков. По набору возможностей система близка к таким коммерческим системам как Maple и Mathematica. В то же время она обладает высочайшей степеньюпереносимости: она может работать на всех основных современных операционных системах на компьютерах, начиная от наладонных компьютеров, вплоть до самых мощных.  
Maxima 拥有最广泛的工具来执行分析计算、数值计算和绘图。就其功能而言，该系统接近Maple和Mathematica等商业系统。同时，它具有最高程度的可移植性：它可以在计算机上的所有主要现代操作系统上运行，从掌上电脑到最强大的操作系统。

Цель работы: исследование возможностей программирования в системах Scilab и Maxima и реализация основных видов алгоритмов.  
工作目的：研究 Scilab 和 Maxima 系统中的编程能力以及主要类型算法的实现。

Объект исследования: системы Scilab и Maxima.  
研究对象：Scilab 和 Maxima 系统。

Предмет исследования: возможности и инструменты программирования в системах Scilab и Maxima.  
研究主题：Scilab 和 Maxima 系统中的编程功能和工具。

Задачи: 任务：

- описать методы реализации базовых алгоритмических структур (следование, ветвление, цикл) и вспомогательных алгоритмов в системе Maxima.  
  描述在 Maxima 系统中实现基本算法结构（跟随、分支、循环）和辅助算法的方法。
- исследовать дополнительные возможности входного языка системы Maxima: создание, трансляция и компиляция функций пользователя  
  探索 Maxima 系统输入语言的附加功能：用户函数的创建、翻译和编译
- рассмотреть функции пользователя как подпрограммы и составить алгоритмы их создания в виде script-файлов в системе Scilab.  
  将用户函数视为子例程，并在 Scilab 系统中以脚本文件的形式创建算法。

### Актуальность
- Актуальность темы заключается в недостаточном объеме линии алгоритмизации программирования в школьном курсе.
- Значимость программирования непосредственно для пользователей, как это демонстрируется в системах Maxima и Scilab.
- Доступность этих систем для свободного использования в сфере образования, что делает их привлекательными для образовательных учреждений.

### Цель работы:
- Исследование возможностей программирования в системах Scilab и Maxima.
- Реализация основных видов алгоритмов.

### Задачи:
- Описание методов реализации базовых алгоритмических структур (следование, ветвление, цикл) и вспомогательных алгоритмов в системе Maxima.
- Исследование дополнительных возможностей входного языка системы Maxima: создание, трансляция и компиляция функций пользователя.
- Рассмотрение функций пользователя как подпрограммы и составление алгоритмов их создания в виде script-файлов в системе Scilab.

### Выводы и рекомендации:
- Оценка преимуществ и недостатков использования Scilab и Maxima в образовании.
- Рекомендации по внедрению этих систем в учебные программы.
...
Проблема защиты авторских прав в России стоит весьма остро. Авторское право очень часто нарушалось и продолжает нарушаться до настоящего времени. На текущий день Россия, к сожалению, остается в числе стран–лидеров по производству и потреблению пиратской продукции.

3. Пиратство распространяется на книги, музыку, фильмы, игры и ПО.
4. Определение интернет-пиратства.

### Разновидности компьютерного пиратства
5. Незаконное копирование пользователями.
6. Незаконная установка ПО на жесткие диски.
7. Изготовление подделок.
8. Нарушение ограничений лицензии.
9. Интернет-пиратство.

### Причины распространения пиратства
10. Экономический аспект: низкий уровень жизни.
11. Технический аспект: доступность современных технологий.

### Влияние интернет-пиратства на издательство
12. Убытки книгоиздателей из-за электронных книг.
13. Пример библиотеки Максима Мошкова.
14. Проблемы контроля распространения произведений в интернете.

### Необходимость реформ авторского права
15. Актуальность авторского права в современных реалиях.
16. Необходимость новых концепций авторского права для интернета.
17. Культурные и этические аспекты отношений и уважение авторских прав.

### Заключение
18. Потребность в изменении сущности копирайта.
19. Важность развития культуры уважения авторских прав.

Задание 4. Составьте аннотацию статьи. Для создания аннотации  пользуйтесь схемой, предложенной в задании 5.

Аннотация

Статья посвящена актуальной проблеме защиты авторских прав в России, где продолжают нарушаться авторские права и страна остаётся лидером по производству и потреблению пиратской продукции. Цель статьи – исследовать феномен интернет-пиратства, его разновидности и влияние на различные сферы, включая книгоиздательство. В статье рассматриваются основные виды интернет-пиратства, такие как незаконное копирование, установка программ, изготовление подделок и нарушение лицензионных ограничений. Автор анализирует экономические и технические причины распространения пиратства, указывая на финансовую неспособность населения покупать лицензионные продукты и техническую оснащенность для их нелегального распространения. Влияние интернет-пиратства на издательское дело иллюстрируется примерами, такими как библиотека Максима Мошкова. Автор приходит к выводу о необходимости реформирования авторского права, адаптируя его к реалиям информационного общества и интернет-культуры. Статья подчеркивает важность создания новых концепций авторского права, основанных на уважении личности и авторских прав в цифровую эпоху.
...
В настоящей статье анализируется...
Методы исследования:
...
## Подготовка к КР по теме "Автобиография и резюме".

По уч.Черкашиной, с. 20:
Задание 28 - Прочитайте описание двух вакансий. Ответьте на вопросы.

1. **Чем должен заниматься инженер-проектировщик слаботочных систем?**
   Инженер-проектировщик слаботочных систем должен заниматься проектированием систем безопасности гражданских и промышленных объектов.
2. **Какие навыки имеет инженер-проектировщик слаботочных систем?**
   Навыки инженера-проектировщика слаботочных систем включают:
   - Высшее техническое образование
   - Опыт работы не менее 3 лет
   - Знание действующей нормативной базы в области проектирования (СП, ПУЭ, нормы, правила и стандарты)
   - Умение оформлять проектно-техническую документацию
   - Знание программ пакета MS Office, AutoCAD и специальных программ
3. **Какую работу должен выполнять архитектор-проектировщик?**

Архитектор-проектировщик должен выполнять следующие задачи:
   - Самостоятельное ведение архитектурной части проекта или отдельной его части
   - Оформление документации

4. **Какие трудовые функции имеет архитектор-проектировщик?**

Трудовые функции архитектора-проектировщика включают:
   - Знание технических, экономических, экологических и социальных требований, предъявляемых к проектируемым объектам
   - Уверенное владение ПК, включая программы Microsoft Office, AutoCAD, Adobe Photoshop и ArchiCAD

Задание 29 - Выберите любую из предложенных вакансий и составьте резюме на выбранную вами должность(с.21)
ВНИМАНИЕ! Здесь указаны только название фирм. Вам нужно будет самим придумать вакансию (название вакансии), связанную именно с вашей специальностью.

Оба, инженер-проектировщик слаботочных систем и архитектор-проектировщик, должны работать по графику 5/2 с 9.00 до 18.00. Это означает, что они работают по 8 часов в день, 5 дней в неделю, с двумя выходными днями.
