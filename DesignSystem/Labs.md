...
开发一个使用计时器控制 LED 亮度和声音发射器的程序（通过中断或使用硬件通道）。程序中不应使用阻塞等待（HAL_Delay()函数）。
...
支架必须通过 UART 与计算机保持通信，并响应计算机键盘上的按钮执行指定的操作。在这个实验室中，每个按键（从计算机发送到支架的符号）都是单独处理的，无需等待输入整行。
...
为了与支架上的 UART 配合使用，表演者可以选择两种驱动程序选项之一（通过中断和轮询）。不需要支持两种变体。
...
处理器核心时钟信号和控制LED亮度的PWM信号（如果使用）的频率必须与任务选项中指定的频率相对应。
...
实现可定制的侧面板 LED 控制面板。遥控器必须有两种主要模式：操作模式和设置模式。

Действия стенда при получении символов от компьютера в рабочем режиме:  
在操作模式下接收来自计算机的符号时支架的动作：

Символ | Действие
-------|---------
«1» - «9» | Зажигание светодиода в соответствии с настройками пульта. Светодиод горит до ввода следующего символа. Предыдущий светодиод выключается. <br> 根据遥控器设置点亮LED。 LED 保持亮起直到输入下一个字符。前一个 LED 熄灭。<br> Настройки по умолчанию:<br>«1» - зеленый, 10 % яркости;<br>«2» - зеленый, 40 % яркости;<br>«3» - зеленый, 100 % яркости;<br>«4» - желтый, 10 % яркости;<br>«5» - желтый, 40 % яркости;<br>«6» - желтый, 100 % яркости;<br>«7» - красный, 10 % яркости;<br>«8» - красный, 40 % яркости;<br>«9» - красный, 100 % яркости.
«0» | Погасить все светодиоды. 关闭所有 LED
«Enter» | Вход в режим настройки. 进入设置模式。
...
进入设置菜单后，您必须首先输入要更改设置的“1”至“9”之间的字符。接下来，符号“a”、“b”或“c”选择 LED（分别为绿色、黄色、红色）。此后，通过多次按下“+”和“-”按钮，填充系数以 10% 的增量从 0 设置为 100%。按下“Enter”按钮，新设置将被保存并转换到操作模式。

По вводу каждого символа в UART должно выводиться сообщение о том, какой режим активирован, или текущие вводимые настройки.  
当每个字符输入到 UART 时，应该显示一条消息，指示激活了哪种模式或正在输入当前设置。

Частота процессорного ядра - 45 МГц, частота ШИМ-сигнала для светодиодов - 250 Гц.  
处理器核心频率为45MHz，LED的PWM信号频率为250Hz。

## 5.4. Лабораторная работа 4. Интерфейс I2C и матричная клавиатура

### Цели работы

1. Получить базовые знания об интерфейсе I2C и особенностях передачи данных по данному интерфейсу.  
   了解I2C接口的基本知识以及该接口传输数据的特点。
...
- нажатие кнопки фиксируется сразу после того, как было обнаружено, что кнопка нажата (с учетом защиты от дребезга), а не в момент отпускания кнопки; если необходимо, долгое нажатие может фиксироваться отдельно;  
  在检测到按钮被按下后立即记录按钮按下动作（考虑到防反弹保护），而不是在按钮被释放时记录；如有需要，长按可单独记录；
- кнопка, которая удерживается дольше, чем один цикл опроса, не считается повторно нажатой до тех пор, пока не будет отпущена (нет переповторов);  
  按下按钮超过一个轮询周期则不认为该按钮被再次按下，直到松开为止（不重复）；
...
该程序必须具有两种操作模式，通过按下支架侧面板上的按钮可以在两种模式之间切换：
...
1. Изучить:  
   ...
   - электрическую принципиальную схему стенда в части сигналов интерфейса I2C и подключения к нему внешних микросхем, а также других устройств, если они требуются по варианту задания;  
     支架的电气原理图，包括 I2C 接口信号和外部微电路的连接，以及其他设备（如果任务选项需要的话）； 
   ...
   - состав стандартных драйверов I2C из библиотеки HAL и содержимое создаваемых генератором файлов i2c.c/.h (появляются в проекте после настройки соответствующих блоков).  
     HAL 库中的标准 I2C 驱动程序的组成以及生成器创建的 i2c.c/.h 文件的内容（设置相应的块后它们会出现在项目中）。
2. Подготовить шаблон проекта для STM32CubeIDE, настроить тактовые частоты.  
   准备STM32CubeIDE的项目模板，设置时钟频率。
3. Настроить входы и выходы микроконтроллера, таймеры, контроллеры I2C и UART.  
   配置微控制器输入和输出、计时器、I2C 和 UART 控制器。
...
5. Разработать необходимые драйверы управляемых светодиодов и/или излучателя звука (зависит от варианта).  
   开发控制 LED 和/或声音发射器所需的驱动程序（取决于选项）。
...
这些任务与实验室工作 3 的选项类似，只是字符的输入不应该通过 UART 从键盘进行，而是使用立式键盘。充当计算机键盘按钮作用的立式键盘按钮的选择应由表演者自行决定。报告必须包括所实施程序中按钮功能的描述。