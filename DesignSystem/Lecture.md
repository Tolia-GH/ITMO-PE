...
#### Общие сведения о GPIO <br> 有关 GPIO 的一般信息
...
#### Пример работы с GPIO в STM32 <br> 在 STM32 中使用 GPIO 的示例
...
На рис. 10 изображена структурная схема аппаратного блока таймера типа TIM2 – TIM5 с 4 каналами ввода-вывода.
...
Рисунок 11 – ШИМ-сигнал: T – период импульсов, d – длительность импульса  
...
После генерации кода остается добавить в программу включение таймера и генерации
...
Рисунок 19 – Бит подтверждения приема байта на шине I2C  
图 19 – I2C 总线字节确认位
...
При взаимодействии с микросхемой адресный байт (рис. 22) содержит постоянную часть и два бита (A1, A0), значения которых определяются в зависимости от того, как подключены соответствующие контакты микросхемы на печатной плате устройства – к земле или питанию. Соответственно, возможно одновременное подключение на шину I2C до 4 микросхем PCA9538. В SDK-1.1M имеется две микросхемы – с адресами 0xE0 (входы прерываний устройств и др.) и 0xE2 (клавиатура). Последний бит адресного байта подчиненного устройства определяет операцию (чтение или запись), которая должна быть выполнена.  
与微电路交互时，地址字节（图22）包含一个常数部分和两个位（A1，A0），其值取决于微电路在印刷电路板上的相应触点如何确定设备已连接至地面或电源。因此，可以同时将最多 4 个 PCA9538 微电路连接到 I2C 总线。 SDK-1.1M 包含两个微电路 - 地址为 0xE0（设备中断输入等）和 0xE2（键盘）。从机地址字节的最后一位决定要执行的操作（读或写）。

![](./pics/lecture1.7-2.png)

Рисунок 22 – Адресный байт PCA9538  
图 22 – PCA9538 地址字节

После успешного подтверждения адресного байта ведущее устройство шины отправляет командный байт (табл. 2). Значение командного байта записывается в регистр управления (control register, биты B7 – B0), и определяет регистр микросхемы, который будет использован далее для чтения или записи.  
成功确认地址字节后，总线主机发送命令字节（表 2）。命令字节的值被写入控制寄存器（位 B7 – B0），并决定进一步用于读取或写入的微电路寄存器。

Таблица 2 – Командный байт [9]  
表 2 – 命令字节[9]

![](./pics/lecture1.7-3.png)
...
登记记录程序。通过发送设备地址并将最低有效位设置为“0”，将数据传输到 PCA9538。在地址字节之后，会发送一个命令字节，该命令字节决定哪个寄存器接收命令字节后的数据（图 23）。

![](./pics/lecture1.7-4.png)

Рисунок 23 – Процедура записи регистра в PCA9538 [9]  
图 23 – PCA9538 中的寄存器写入过程 [9]

Процедура чтения регистров. Сначала ведущее устройство шины должно отправить адресный байт PCA9538 с младшим значащим битом, установленным в «0» (запись). После адреса отправляется командный байт, и определяет, к какому регистру обращаются. После этого на шине I2C формируется последовательность «повторный старт», снова отправляется адресный байт, но на этот раз младший значащий бит устанавливается в «1» (чтение). После этого данные из регистра, определенного командным байтом, читаются из PCA9538 (рис. 24). На количество байтов данных, полученных в одной передаче чтения, ограничения нет (можно прочитать все 4 регистра подряд). Когда ведущее устройство читает последний нужный ему байт, оно не должно подтверждать данные.  
读取寄存器的程序。首先，总线主机必须发送一个 PCA9538 地址字节，其中最低有效位设置为“0”（写入）。地址之后，会发送一个命令字节并确定正在访问哪个寄存器。此后，I2C 总线上形成一个“重启”序列，地址字节再次被发送，但这一次最低有效位被设置为“1”（读取）。此后，从PCA9538读取命令字节指定的寄存器的数据（图24）。一次读取传输中接收的数据字节数没有限制（您可以连续读取所有 4 个寄存器）。当主机读取了它所需的最后一个字节时，它不必确认该数据。

![](./pics/lecture1.7-5.png)

Рисунок 24 – Процедура чтения регистров из PCA9538 [9]  
图 24 – 从 PCA9538 读取寄存器的过程 [9]
...
输入端口寄存器（寄存器 0）反映芯片引脚的传入逻辑电平，无论该引脚是否被配置寄存器定义为输入或输出。仅影响读取操作。写入该寄存器没有任何效果。
...
输出端口寄存器（寄存器 1）设置由配置寄存器定义为输出的引脚的输出逻辑电平。该寄存器中的位值不影响定义为输入的引脚。反过来，从该寄存器读取会返回控制触点的触发器中的值，而不是触点处的实际信号电平。
...
极性反转寄存器（寄存器 2）允许您反转定义为配置寄存器输入的信号的极性。如果设置该寄存器中的某个位（写入“1”），则相应输出端口的极性将反转。如果该寄存器中的某个位被清除（写入“0”），则相应输出端口的极性将被保留。
...
配置寄存器（寄存器 3）设置输入/输出的方向。如果此寄存器中的某个位设置为“1”，则相应的触点将被激活为输入。如果此寄存器中的某个位设置为“0”，则相应的触点将被激活为输出。

#### Обмен данными с расширителем портов ввода-вывода <br> 与 I/O 端口扩展器进行数据交换

Для чтения и записи по интерфейсу I2C регистров в микросхемах, которые требуют установки адреса внутреннего регистра (например, микросхема PCA9538), в библиотеке HAL существуют функции, который сразу выполняют все необходимые действия:  
为了在需要设置内部寄存器地址的芯片（例如 PCA9538 芯片）中通过 I2C 接口读取和写入寄存器，HAL 库包含可立即执行所有必要操作的函数：
...
这些参数与上面描述的类似，并且名称相同。新参数：
- MemAddress – адрес внутри ведомого устройства I2C, который записывается в устройство после передачи адресного байта (например, номер регистра PCA9538);  
  MemAddress – I2C 从设备内部的地址，传输地址字节后写入设备（例如 PCA9538 寄存器号）；
- MemAddSize – размер адреса внутри ведомого устройства в байтах; может быть равен 1 или 2 (для PCA9538 равен 1).  
  MemAddSize – 从设备内部地址的大小（以字节为单位）；可以等于 1 或 2（对于 PCA9538，它等于 1）。

Также существуют варианты этих функций, работающие в режиме прерывания аналогично драйверу UART. Чтобы их использовать, необходимо разрешить прерывания I2C в настройках NVIC.  
这些函数还有以类似于 UART 驱动程序的中断模式运行的变体。要使用它们，您需要在 NVIC 设置中启用 I2C 中断。

#### Варианты организации драйвера клавиатуры <br> 键盘驱动程序组织选项
...
1. Работа с I2C по опросу (без прерываний от I2C). Опрос клавиатуры периодически выполняется из главного цикла while() в функции main(). При этом остальной код главного цикла должен отдавать управление достаточно быстро, чтобы не создавать заметных пауз в опросе клавиатуры.  
   通过轮询使用 I2C（无需来自 I2C 的中断）。键盘从 main() 函数中的主 while() 循环定期轮询。同时，主循环代码的其余部分必须足够快地返回控制，以免在键盘轮询中产生明显的暂停。
