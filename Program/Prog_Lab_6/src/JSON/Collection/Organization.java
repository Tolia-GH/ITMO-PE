...
public class Organization implements Comparable<Organization> {
     ...
     */
    private JSON.Collection.OrganizationType type; //Поле может быть null
    /**
     * The Postal address of Organization.
     */
    private JSON.Collection.Address postalAddress; //Поле не может быть null
    ...
    public static Organization Create() throws NullValueException, ValueOutOfRangeException {
        ...
        JSON.Collection.OrganizationType type;
        Tools.MessageL("    Set organization type from: ");
        Tools.MessageL(JSON.Collection.OrganizationType.list());
        Tools.Message("    Organization type = ");
        String typeInput = Tools.Input();
        if (typeInput.equals("")) {
            type = JSON.Collection.OrganizationType.NULL;
        } else {
            if (JSON.Collection.OrganizationType.findType(typeInput)) {
                type = JSON.Collection.OrganizationType.valueOf(typeInput);
            } else {
        ...
        JSON.Collection.Address postalAddress = new JSON.Collection.Address(street, zipCode);
     ...
     * @throws NullValueException the null value exception
     */
    public void setType(JSON.Collection.OrganizationType type) throws NullValueException {
        if (type.equals(JSON.Collection.OrganizationType.NULL)) {
     ...
     * @throws NullValueException the null value exception
     */
    public void setPostalAddress(JSON.Collection.Address postalAddress) throws NullValueException {
