...
为了确定频率上限，需要找到传输消息中频谱的最高频率分量。 在NRZ码中，高频分量是通过传输交替值 0 和 1 而形成的，用于传输方波信号0和1的正弦波（谐波信号）的周期将等于位间隔 $t：T = 2t$，其中 $t$ 定义为数据传输速率（通道容量）的倒数：$t = 1⁄C$。 因此，频率上限将等于 $f = 1⁄T = C⁄2$。 当通信信道容量为 $C = 1$ Mbit⁄s 时，基波谐波的频率等于 $f_v = 500$ kHz。
...
$$f_{ср} =(7f_0 +4f_0 /2+8f_0 /2)/19\approx 0,58f_0 =290\ кГц$$.
